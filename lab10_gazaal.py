...
Your answer is: None (no return even_list)
...
Your answer is: Yes, because both functions are traverssing through the list until the num in nums is equal to the target, and once 
its equal to the target it will print the index, if the number in the list never reaches the number in the target, then it returns index of -1
...
functionality? Why? (5 marks)
Your answer is: No, because  practice_for_index_2 does not have the return statement in the for loop and it breaks the loop when it reaches index 3, however 
practice_for_index_3 keeps traverssing the loop until it reaches index 4 because the loop does not break
...
assert exist_in_both("apple", "streak") == 2
...
assert more_than_2(["hi", "hi", "bye", "hi"]) == ["hi"]
